...
export const storeUpdateAPI = async (values: TinitialValues) => {
  ...
  if (values.newStoreBrcImages) {
    Array.from(values.newStoreBrcImages).forEach((img) => {
      formData.append("storeBrcImages", img);
    });
  }
  if (values.storeName) {
  ...
  if (values.storeContactNumber) {
    formData.append("storeContactNumber", values.storeContactNumber);
  }

  if (values.storeHompageUrl) {
    formData.append("storeHompageUrl", values.storeHompageUrl);
  }

  if (values.storeExplanation)
    formData.append("storeExplanation", values.storeExplanation);
